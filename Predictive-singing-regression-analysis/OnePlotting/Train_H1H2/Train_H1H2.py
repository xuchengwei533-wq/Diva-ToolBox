...
output_dir = os.path.join(output_root, "Train_H1H2")
...
def build_dataset():
    if not os.path.isdir(h1h2_dir):
        raise FileNotFoundError(f"特征目录不存在: {h1h2_dir}")
    files = [f for f in os.listdir(h1h2_dir) if f.lower().endswith(".csv")]
    files.sort()
    X = []
    y = []
    kept = []
    for f in files:
        ...
        if not os.path.exists(h1h2_path):
            continue
        h1h2_stats = stats_mean_std(load_series_csv(h1h2_path))
        if h1h2_stats is None:
            continue
        feat = np.array([h1h2_stats[0], h1h2_stats[1]], dtype=np.float32)
        X.append(feat)
        y.append(label)
        kept.append(base)

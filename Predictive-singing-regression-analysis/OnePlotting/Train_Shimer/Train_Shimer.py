...
def build_dataset():
    if not os.path.isdir(shimmer_dir):
        raise FileNotFoundError(f"特征目录不存在: {shimmer_dir}")
    files = [f for f in os.listdir(shimmer_dir) if f.lower().endswith(".csv")]
    files.sort()
    X = []
    y = []
    kept = []
    for f in files:
        ...
        if not os.path.exists(shimmer_path):
            continue
        shimmer_stats = stats_mean_std(load_series_csv(shimmer_path))
        if shimmer_stats is None:
            continue
        feat = np.array([shimmer_stats[0], shimmer_stats[1]], dtype=np.float32)
        X.append(feat)
        y.append(label)
        kept.append(base)

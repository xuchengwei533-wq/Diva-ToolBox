...
Shimmer_out = os.path.join(output_root, "Shimmer")
os.makedirs(Shimmer_out, exist_ok=True)
...
print(f"📄 Shimmer 输出目录: {Shimmer_out}")
print("-" * 60)
...
if HAS_TQDM:
    pbar = tq.tqdm(wav_files, total=len(wav_files), desc="提取Shimmer", unit="file", dynamic_ncols=True)
    for idx, file in enumerate(pbar, start=1):
        start_t = time.perf_counter()
        in_path = os.path.join(Audio_path, file)
        out_name = os.path.splitext(file)[0] + ".csv"
        out_path = os.path.join(Shimmer_out, out_name)
        if os.path.exists(out_path):
            pbar.set_postfix({"步骤": "跳过(已存在)", "文件": file})
            continue
        audio, original_sr, target_sr = load_audio(in_path)
        pbar.set_postfix({"步骤": "提取Shimmer", "文件": file})
        shimmer = extract_shimmer(audio, target_sr)
        if shimmer is None:
            tq.tqdm.write(f"❌ 提取失败: {file}")
            continue
        np.savetxt(out_path, shimmer, delimiter=",", fmt="%.6f")
        cost_s = time.perf_counter() - start_t
        pbar.set_postfix({"步骤": f"完成({cost_s:.1f}s)", "文件": file})
        tq.tqdm.write(f"✅ [{idx}/{len(wav_files)}] {file} | Shimmer saved: {out_path}")
else:

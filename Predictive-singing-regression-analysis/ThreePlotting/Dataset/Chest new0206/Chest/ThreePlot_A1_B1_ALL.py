    ...
    for i in range(len(FEATURES)):
        for j in range(i + 1, len(FEATURES)):
            for k in range(j + 1, len(FEATURES)):
                ...
                zlim = compute_axis_limits(all_zs)
                
                fig = plt.figure(figsize=(12, 4), dpi=150)
                
                ax1 = fig.add_subplot(131, projection='3d')
                point_size = 22 if "Jitter" in (feature_x["name"], feature_y["name"], feature_z["name"]) else 30
                scatter_feature_triplet(ax1, a1_xs, a1_ys, a1_zs, a1_scores, "A1", feature_x["label"], feature_y["label"], feature_z["label"], xlim, ylim, zlim, point_size)
                
                ax2 = fig.add_subplot(132, projection='3d')
                scatter_feature_triplet(ax2, b1_xs, b1_ys, b1_zs, b1_scores, "B1", feature_x["label"], feature_y["label"], feature_z["label"], xlim, ylim, zlim, point_size)
                
                ax3 = fig.add_subplot(133, projection='3d')
                scatter_feature_triplet(ax3, all_xs, all_ys, all_zs, all_scores, "ALL", feature_x["label"], feature_y["label"], feature_z["label"], xlim, ylim, zlim, point_size)
